...
def pca(X):
    ...
    Input: X, a matrix of flattened arrays in rows.
    Output: projection matrix (with important dimensions first), varience and mean
    """
    ...
    if dim> num_data:
        ...
        V = tmp[::-1]#reverse since the last eigenvectors are the desired ones
        S = sqrt(e)[::-1]#reverse as the eigenvalues are in increasing order
        ...
        #PCA SVD used
        U,S,V= linalg.svd(X)
        V = V[:num_data]#only want to return the first num_data
